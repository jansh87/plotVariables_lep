...
//	vVar.push_back(tmp);
	...
	tmp = CVar("mm2g","M_{miss}^{2} / GeV^{2}",&gmiss.m2, 70,-5, 15);
	vVar.push_back(tmp);
...
*/
			...
			//if(gmiss.m2<0) cont = signal_decay;
			//else cont = charm_lep;
		//	else cont = charm_lep;
	...
	for(vector<CFile>::iterator it_f = vFile.begin(); it_f != vFile.end(); ++it_f)
	{
		...
		*/fCount++;
	    ...
	    //string f_newName = string("/scratch/bctaunu/kakuno/new_0815/small/")+it_f->Name;	
            string f_newName = string("~/small_")+it_f->Name;	
...
*/
		...
		//	if(run.exp>37) continue;
           ...
           // if(1.776-lep1.ps>gmiss.ps) {nCut++;continue;}
			...
			//if(it_f->Type == 's') gx.m2 = sqrt(gx.m2);
			...
			*/int nTracks = 0;
			...
			for(; itp1!=vtrk_p1->end();++itp1, ++itp2, ++itp3, ++ite, ++itsep,++itdr, ++itdz, ++itq/*,++itp1_gen, ++itp2_gen, ++itp3_gen, ++ite_gen, ++itid_gen, ++itis_gen, ++it_moid_gen,++itkpid*/)
			{
				...
				*/
				...
				//if(mp_tmp.pid_true ==5)
				//	mp_tmp.P.SetE(sqrt(sqr(0.938) + sqr(mp_tmp.P.Rho())));
...
*/
			...
			//if(cont != data)
			//	MC_Correction *= EgammaCorr[hists[0][iEgammaIdx]->FindBin(lvMaxGamma.E())];
...
//	*/	lvBreco.Boost(-lvBreco.BoostVector());
	...
	for(int i=0; i<2; i++)
	{
		hOverlap[i]->Scale(1./hOverlap[i]->Integral());
		hOverlap[i]->Draw("same");
	}
	c2->Print("Overlap.pdf");
	float bestDiff = 9999.;
	float bestIdx;
	for(int i = 1; i<=hOverlap[0]->GetNbinsX(); i++)
	{
		float i1 = hOverlap[0]->Integral(i,hOverlap[0]->GetNbinsX());
		float i2 = hOverlap[1]->Integral(1,i);
		float diff = fabs(i1-i2);
		cout<<i<<" "<<i1<<" "<<i2<<endl;
		if(diff<bestDiff){bestDiff=diff; bestIdx=i;}
	}
	cout<<"Overlap:diff:idx:value "<<bestDiff<<" "<<bestIdx<<" "<<hOverlap[0]->GetBinCenter(bestIdx)<<endl;
