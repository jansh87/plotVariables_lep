...
//	vVar.push_back(tmp);
	...
	tmp = CVar("ps1e","electron p*_{#tau} / GeV",&lep1.pc, 0.3, 2.5);
	vVar.push_back(tmp);
	nVarPs1Elec = vVar.size();

	tmp = CVar("ps1mu","muon p*_{#tau} / GeV",&lep1.pc, 0.5, 2.5);
	vVar.push_back(tmp);
...
*/
	...
	tmp = CVar("mm2g","M_{miss}^{2} / GeV^{2}",&gmiss.m2, 70,-10, 25);
	vVar.push_back(tmp);

	tmp = CVar("mm2gnPeak","M_{miss}^{2} / GeV^{2}",&gmiss.m2, 30,-5, 15);
	vVar.push_back(tmp);

	tmp = CVar("mm2gtrans","M_{miss}^{2} / GeV^{2}",&gmiss.m2,90, -5, 15);
	vVar.push_back(tmp);
...
//		if(lep1.ps<1.2 ) continue;
		...
		//	if(sqrt(xlep.m2)<2.) continue;
			...
			//if(gmiss.m2>0)	
				//MC_Correction*=mm2_corr2(gmiss.m2);
...
//		lvBreco.Boost(-lvBreco.BoostVector());
		...
		t_PmissWoGamma.costh = lvPmissWoGamma.CosTheta();
		   ...
		   // for(auto it= vParticles.begin(); it!=vParticles.end();++it)
			...
			if(cont != data)
			{
					h2d[1]->Fill(lep1.ps,gmiss.m2,MC_Correction*it_f->weight*mm2_corr(gmiss.m2));
			//	}else{
					h2d[0]->Fill(lep1.ps,gmiss.m2,MC_Correction*it_f->weight);
					//if(cont == charm_lep || cont == charm_s_lep )
						h2d[3]->Fill(lep1.ps,gmiss.m2,MC_Correction*it_f->weight);
				//	else 
						h2d[4]->Fill(lep1.ps,gmiss.m2,MC_Correction*it_f->weight);
